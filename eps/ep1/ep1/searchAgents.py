...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        if self.startingPosition in self.corners:
            cantos_visitados = self.startingPosition
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                proxima_posicao = (nextx, nexty)
                proximo_canto = state[1]
                if proxima_posicao in self.corners and proxima_posicao not in state[1]:
                    proximo_canto = (proxima_posicao,) + state[1]
                proximo_estado = (proxima_posicao, proximo_canto)
                successors.append((proximo_estado, action, 1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    while len(lista) > 0:
        for corner in lista:
            distancia = manhattanDistance(corner, referencia)
            if distancia < distancia_minima:
                indice = corner
                distancia_minima = distancia
        total += distancia_minima
        distancia_minima = float('inf')
